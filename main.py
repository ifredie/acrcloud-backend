...
def generar_excel(data: dict):
    ...
    # Segunda hoja: resumen por día y por stream
    resumen_ws = wb.create_sheet("Resumen Diario")
    resumen_ws.append(["Fecha", "Stream", "Pautados", "Faltantes", "Fuera de horario", "Total"])

    resumen_dict = {}
    for item in data.get("detected", []):
        clave = (item["fecha"], item["stream"])
        resumen = resumen_dict.setdefault(clave, {"Pautados": 0, "Faltantes": 0, "Fuera": 0})
        if item["estado"] == "DETECTADO":
            resumen["Pautados"] += 1
        elif item["estado"] == "FUERA DE HORARIO":
            resumen["Fuera"] += 1
    ...
    for (fecha, stream), resumen in resumen_dict.items():
        total = resumen["Pautados"] + resumen["Faltantes"] + resumen["Fuera"]
        resumen_ws.append([
            fecha,
            stream,
            resumen["Pautados"],
                    ...
                    for item in deteccion.get("metadata", {}).get("custom_files", []):
                        if item.get("acrid") == material.acr_id:
                            ...
                            try:
                                ...
                                dt_local = (dt_utc - timedelta(hours=6)).replace(second=0, microsecond=0)
    ...
    for material in payload.materiales:
        for fecha in material.fechas:
            for hora in material.horarios:
                hora_objetivo_dt = datetime.strptime(f"{fecha} {hora}", "%Y-%m-%d %H:%M").replace(second=0, microsecond=0)
                detectado = None

                for r in resultados:
                    if (
                        r["acr_id"] == material.acr_id and
                        r["stream"] in material.stream_ids and
                        abs((r["hora_dt"] - hora_objetivo_dt).total_seconds()) <= payload.tolerancia_minutos * 60 and
                        (r["acr_id"], hora_objetivo_dt, r["stream"]) not in ya_detectados
                    ):
                        detectado = {
                            **r,
                            "hora_pautada": hora,
                        ...
                        ya_detectados.add((r["acr_id"], hora_objetivo_dt, r["stream"]))
                        break
                ...
                if detectado:
                    ...
                    faltantes.append({
                        ...
                        "hora_pautada": hora,
                        "acr_id": material.acr_id,
                        "stream": material.stream_ids[0]  # tomar el primero si no está individualizado
                    })

    # Marcar el resto como fuera de horario si no fueron ya detectados
    for r in resultados:
        if (r["acr_id"], r["hora_dt"], r["stream"]) not in ya_detectados:
