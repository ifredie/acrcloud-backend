...
async def get_results_from_acrcloud(project_id: str, stream_id: str, fecha_yyyymmdd: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            return {"error": response.text, "codigo": response.status_code, "detalle": f"Error en stream {stream_id} con fecha {fecha_yyyymmdd}"}
...
async def generar_reporte(payload: ProyectoRequest):
    ...
    # Generar lista de faltantes considerando tolerancia
    faltantes = []
    for material in payload.materiales:
        for fecha in material.fechas:
                ...
                for stream_id in material.stream_ids:
                    ...
                    for r in resultados:
                        if (
                            ...
                            if hora_detectada_dt.hour == hora_objetivo_dt.hour and
